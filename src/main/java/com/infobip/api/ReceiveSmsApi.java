...
public class ReceiveSmsApi {
  ...
  /**
   * Get inbound SMS messages (asynchronously) If for some reason you are unable to receive incoming
   * SMS at the endpoint of your choice in real time, you can use this API call to fetch messages.
   * Each request will return a batch of received messages - only once. The following API request
   * will return only new messages that arrived since the last API request.
   *
   * @param limit Maximal number of received messages that will be returned. (optional)
